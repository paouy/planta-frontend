...
<template>
  <CfAppLayout>
    ...
    <template #sidebar>
      ...
      <!-- <CfSidebarLink label="Settings" icon="settings" path="/settings" v-if="session.user?.isAdmin">
        <template #children>
          <CfSidebarLink label="Users" :to="{ name: 'Users' }"/>
          <CfSidebarLink label="Configurations" :to="{ name: 'Configurations' }"/>
        </template>
      </CfSidebarLink> -->
    </template>

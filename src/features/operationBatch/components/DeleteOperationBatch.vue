...
const dialogTitle = computed(() => `Delete ${props.operation.name.toLowerCase()} batch`)
...
const invoke = async () => {
  try {
    ...
    await api.operationBatch.deleteOne(props.data.id)
...
<template>
  <CfDialog :title="dialogTitle" :persist="isLoading" @close="emit('cancel')">
    <template #body>
      <p>You are about to delete <b>{{ props.data.publicId }} — {{ props.data.schedule }}</b> with {{ normalizedJobCount }}. Do you want to proceed?</p>
    </template>
    <template #footer>
